...
const (
	// DL20 is a HPE Hardware Device we scrape
	DL20 = "DL20"
...
func (e *Exporter) scrape() {
	...
	for _, task := range e.pool.Tasks {
		...
		if err != nil {
			scrapeChan <- 0
			log.Error("error exporting metrics - from "+DL20, zap.Error(err), zap.String("api", task.MetricType), zap.Any("trace_id", e.ctx.Value("traceID")))
			continue
		}
		scrapeChan <- 1
	}
