...
const (
	// DL560 is a HPE Hardware Device we scrape
	DL560 = "DL560"
...
// NewExporter returns an initialized Exporter for HPE DL560 device.
func NewExporter(ctx context.Context, target, uri, profile string) (*Exporter, error) {
	...
	if err != nil {
		log.Error("error when getting chassis endpoint from "+DL560, zap.Error(err), zap.Any("trace_id", ctx.Value("traceID")))
	...
	for _, member := range driveResp.Members {
		...
		if arrayCtrlResp.LinksUpper.LogicalDrives.URL != "" {
			logicalDrives, err := getDriveEndpoint(fqdn.String()+arrayCtrlResp.LinksUpper.LogicalDrives.URL, target, retryClient)
			if err != nil {
			...
			for _, member := range logicalDrives.Members {
		...
		if arrayCtrlResp.LinksUpper.PhysicalDrives.URL != "" {
			physicalDrives, err := getDriveEndpoint(fqdn.String()+arrayCtrlResp.LinksUpper.PhysicalDrives.URL, target, retryClient)
			if err != nil {
			...
			for _, member := range physicalDrives.Members {
...
func (e *Exporter) scrape() {
	...
	for _, task := range e.pool.Tasks {
		...
		if task.Err != nil {
			...
			log.Error("error from "+DL560, zap.Error(task.Err), zap.String("api", task.MetricType), zap.Any("trace_id", e.ctx.Value("traceID")))
			return
		}
		...
		if err != nil {
			log.Error("error exporting metrics - from "+DL560, zap.Error(err), zap.String("api", task.MetricType), zap.Any("trace_id", e.ctx.Value("traceID")))
			continue
		}
		scrapeChan <- 1
	}
...
func (e *Exporter) exportThermalMetrics(body []byte) error {
	...
	if err != nil {
		return fmt.Errorf("Error Unmarshalling DL560 ThermalMetrics - " + err.Error())
	}
...
func (e *Exporter) exportPhysicalDriveMetrics(body []byte) error {
	...
	if err != nil {
		return fmt.Errorf("Error Unmarshalling DL560 DiskDriveMetrics - " + err.Error())
	}
...
func (e *Exporter) exportNVMeDriveMetrics(body []byte) error {
	...
	if err != nil {
		return fmt.Errorf("Error Unmarshalling DL560 NVMeDriveMetrics - " + err.Error())
	}
...
func (e *Exporter) exportMemorySummaryMetrics(body []byte) error {
	...
	if err != nil {
		return fmt.Errorf("Error Unmarshalling DL560 MemorySummaryMetrics - " + err.Error())
	}
...
func getDIMMEndpoints(url, host string, client *retryablehttp.Client) (oem.Collection, error) {
	...
	if err != nil {
		return dimms, fmt.Errorf("Error Unmarshalling DL560 Memory Collection struct - " + err.Error())
	}
...
func getDriveEndpoint(url, host string, client *retryablehttp.Client) (oem.GenericDrive, error) {
	...
	if err != nil {
		return drive, fmt.Errorf("Error Unmarshalling DL560 Drive Collection struct - " + err.Error())
	}
