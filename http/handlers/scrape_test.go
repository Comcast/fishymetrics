...
func Test_ScrapeHandler_InvalidProxyHostDoesNotRunCredentialsScript(t *testing.T) {
	marker := filepath.Join(t.TempDir(), "script-ran")
	script := writeCredentialsScript(t, "echo ran > "+marker+"\necho '{\"user\":\"root\",\"pass\":\"toor\"}'\n")
	...
	if _, err := os.Stat(marker); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("script marker stat error = %v, want not exist", err)
	}
}
